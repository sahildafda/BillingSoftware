...
// Optional firm/GST details on customers, snapshotted onto invoices at billing time
ensureColumn("customers", "firmName", "TEXT NOT NULL DEFAULT ''");
...
async function sendInvoiceWhatsApp(invoice, customer, user) {
    ...
    if (!enabled) {
        console.log(
            "[WhatsApp] Twilio WhatsApp delivery is disabled; set TWILIO_WHATSAPP_ENABLE=true to enable it."
        );
    ...
    const totalAmount = formatCurrency(invoice?.total || 0);
    ...
    try {
        ...
        const result = await client.messages.create({
            ...
            contentSid: "HX385da02348ac2d73a6bd66e3e3f34f2b",
        ...
        console.log("[WhatsApp] Invoice template message sent successfully:", result.sid);
        ...
        console.error(
            "[WhatsApp] Invoice message delivery failed:",
            error.message
        );
...
async function sendInvoiceSMS(invoice, customer, user) {
    const accountSid = process.env.TWILIO_ACCOUNT_SID || "";
    const authToken = process.env.TWILIO_AUTH_TOKEN || "";
    ...
    if (!accountSid || !authToken || !from || !to) {
        console.log("[SMS] Missing Twilio configuration or customer phone number.");

        return {
            success: false,
            skipped: true,
            reason: "Missing Twilio SMS configuration or customer phone"
        };
    }

    const customerName =
        String(customer?.customerName || "Customer").trim() || "Customer";

    const businessName =
        String(user?.companyName || "Billing Software").trim() ||
        "Billing Software";
    ...
    const message =
        ...
        `of ${total} has been generated by ${businessName}. ` +
        `Detailed invoice has been sent to your email. Thank you!`;
    ...
    try {
        ...
        console.log("[SMS] Invoice SMS sent:", result.sid);
        ...
        console.error("[SMS] Invoice SMS failed:", error.message);

        return {
            success: false,
            error: error.message,
            reason: "Twilio SMS delivery failed"
        };
    }
}

function generateInvoicePdf(invoice, customer, user) {
    return new Promise((resolve, reject) => {
        ...
        const customerName =
            String(customer?.customerName || invoice?.customerName || "Customer").trim();
        ...
        if (customer?.contactNumber) {
            doc.text(`Phone: ${customer.contactNumber}`);
        }
        ...
        if (customer?.firmName) {
            doc.text(`Firm: ${customer.firmName}`);
        }

        if (customer?.gstNo) {
            doc.text(`GST No: ${customer.gstNo}`);
        }
...
async function sendInvoiceEmail(invoice, customer, user, pdfBuffer) {
    const email = String(
        customer?.email ||
        invoice?.email ||
        ""
    ...
    if (!email) {
        console.log("[Email] Customer does not have an email address.");
    ...
    if (!process.env.EMAIL_USER || !process.env.EMAIL_PASS) {
        console.log("[Email] EMAIL_USER / EMAIL_PASS are missing.");

        return {
            success: false,
            skipped: true,
            reason: "Email SMTP configuration is missing"
        };
    }
        ...
        <body style="font-family:Arial,sans-serif;background:#f5f5f5;padding:30px;">
            ...
            ">
                ...
                <div style="margin-top:25px;text-align:right;">
                    ...
                    <p>GST: ${formatCurrency(invoice.gstTotal)}</p>
                    <p>Discount: ${formatCurrency(invoice.discountTotal)}</p>
    ...
    try {
        ...
        return {
            success: false,
            error: error.message,
            reason: "Invoice email delivery failed"
        };
    }
}
...
app.post("/api/billing/invoices", authenticateToken, async (req, res) => {
    ...
    const normalizedFirmName = customer && customer.firmName ? String(customer.firmName).trim() : "";
    const normalizedGstNo = customer && customer.gstNo ? String(customer.gstNo).trim() : "";
...
app.get("/api/products/barcode/:barcode", authenticateToken, (req, res) => {
    try {
        ...
        return res.status(500).json({
            message: "Unable to find product by barcode",
        });
    }
});
...
app.post("/api/customers", authenticateToken, (req, res) => {
    const { customerName, contactNumber, email, credit, firmName, gstNo } = req.body;
    ...
    const normalizedGstNo = String(gstNo || "").trim();
...
app.put("/api/customers/:id", authenticateToken, (req, res) => {
    ...
    if (req.body.firmName !== undefined) {
        updates.push("firmName = ?");
        values.push(String(req.body.firmName || "").trim());
    }

    if (req.body.gstNo !== undefined) {
        updates.push("gstNo = ?");
        values.push(String(req.body.gstNo || "").trim());
    }
